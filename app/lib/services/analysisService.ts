export async function callClaudeAPI(prompt: string, apiKey: string, stream: boolean = false, pdfData?: string) {
    ...
    const allModels = [
        { id: 'claude-sonnet-4-5-20250929', maxTokens: 8000, supportsPdf: true },
        { id: 'claude-haiku-4-5-20251001', maxTokens: 8000, supportsPdf: true },
        { id: 'claude-3-5-sonnet-20241022', maxTokens: 8000, supportsPdf: true },
        { id: 'claude-3-5-haiku-20241022', maxTokens: 4000, supportsPdf: true },
    ]

    // Filter models based on capabilities
    const models = allModels.filter(m => !pdfData || m.supportsPdf)
